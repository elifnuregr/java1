...
public class MayinTarlasi {
    ...
    public void Run() {
        ...
        createField(cizgi, sutun);// Boş tarla oluşturma
        AddMines(cizgi, sutun); // Mayın Ekleme
        ShowField(cizgi, sutun); // Tarlanın Mayınlı Halini Gösterme
    ...
    public void createField(int cizgi,int sutun) // Tarlayı oluşturma
    {
        matrix = new char[cizgi][sutun];//normal dizi mayınsız
        NumField=new String[cizgi][sutun]; // kullanıcıya gösterilecek dizi
        for (int i=0;i<cizgi;i++){
            for (int j=0;j<sutun;j++){
                matrix[i][j]='-';
                NumField[i][j]="-";
